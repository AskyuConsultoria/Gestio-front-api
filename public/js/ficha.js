...
async function listarBotaoPeca() {
    ...
    const data = await fetch(`http://192.168.15.3:8080/pecas/${idUsuario}`);
        if (!data.ok) {
...
async function listarFichas(){
    const idUsuario = sessionStorage.getItem('id')
    const data = await fetch(`http://192.168.15.3:8080/itens-pedidos/${idUsuario}`);
        if (!data.ok) {
        throw new Error('Erro ' + data.statusText);
        }
...
async function listarClientes(){
    ...
    const idUsuario = sessionStorage.getItem('id')
    const data = await fetch(`http://192.168.15.3:8080/clientes/${idUsuario}`);
        if (!data.ok) {
        throw new Error('Erro ' + data.statusText);
        }
...
async function buscarDadosCliente(){
    ...
    const data = await fetch( `http://192.168.15.3:8080/clientes/${idCliente}/buscarUm`);
    if (!data.ok) {
        throw new Error('Erro ' + data.statusText);
    }
