...
async function deletarFicha(evento, fichaId){
    ...
    try{
        var response = await fetch(`/askyuitens-pedidos/${usuarioId}/${fichaId}`, {
            method: "DELETE"
        })
...
async function listarBotaoPeca() {
    ...
    const data = await fetch(`/askyupecas/${idUsuario}`);
        if (!data.ok) {
...
async function listarFichas(){
    const idUsuario = sessionStorage.getItem('id')
    const data = await fetch(`/askyuitens-pedidos/${idUsuario}`);
        if (!data.ok) {
        throw new Error('Erro ' + data.statusText);
        }
...
async function criarPedido(){
    ...
    try{
        const response = await fetch("/askyupedido", {
            method: "POST",
            headers: {
...
async function buscarItemPedido(){
    ...
    try{
        const response =  await fetch(`/askyuitens-pedidos/${usuarioId}/${itemPedidoId}/buscar-um`, {
            method: "GET"
        })
...
async function listarClientes(){
    ...
    const idUsuario = sessionStorage.getItem('id')
    const data = await fetch(`/askyuclientes/${idUsuario}`, {
        method: 'GET'
    });
        if (!data.ok) {
...
async function buscarDadosCliente(){
    ...
    const data = await fetch( `/askyuclientes/${idCliente}/buscarUm`);
    if (!data.ok) {
        throw new Error('Erro ' + data.statusText);
    }
...
async function buscarDadosFicha(itemPedidoId){
    ...
    try{
        var response = await fetch(`/askyuitens-pedidos/${usuarioId}/${itemPedidoId}/buscar-um`, {
            method: "GET"
        })
