...
async function listarBotaoPeca() {
    ...
    const data = await fetch(`http://192.168.137.199:8080/pecas/${idUsuario}`);
        if (!data.ok) {
...
async function listarFichas(){
    const idUsuario = sessionStorage.getItem('id')
    const data = await fetch(`http://192.168.137.199:8080/itens-pedidos/${idUsuario}`);
        if (!data.ok) {
        throw new Error('Erro ' + data.statusText);
        }
...
async function criarPedido(){
    ...
    try{
        const response = await fetch("http://192.168.137.199:8080/pedido", {
            method: "POST",
            headers: {
...
async function buscarItemPedido(){
    ...
    try{
        const response =  await fetch(`http://192.168.137.199:8080/itens-pedidos/${usuarioId}/${itemPedidoId}/buscar-um`, {
            method: "GET"
        })
...
async function validarAssociacaoPedido(itemPedidoId) {
    ...
    console.log(pedido)
    location.assign("http://192.168.137.199:3333/associar-ficha.html")
}
...
async function IrVisualizarFicha(elFicha){
    if(sessionStorage.getItem("CADASTRO-PEDIDO")){
        ...
        location.assign("http://192.168.137.199:3333/fichas/vincular_medidas.html")
    }
...
async function listarClientes(){
    ...
    const idUsuario = sessionStorage.getItem('id')
    const data = await fetch(`http://192.168.137.199:8080/clientes/${idUsuario}`, {
        method: 'GET'
    });
        if (!data.ok) {
...
async function buscarDadosCliente(){
    ...
    const data = await fetch( `http://192.168.137.199:8080/clientes/${idCliente}/buscarUm`);
    if (!data.ok) {
        throw new Error('Erro ' + data.statusText);
    }
...
async function buscarDadosFicha(itemPedidoId){
    ...
    try{
        var response = await fetch(`http://192.168.137.199:8080/itens-pedidos/${usuarioId}/${itemPedidoId}/buscar-um`, {
            method: "GET"
        })
