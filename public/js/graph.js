async function buscarViewPedido(usuarioId){
    try {
        const resposta = await fetch(`http://10.0.1.226:8080/askyu/pedido-view/${usuarioId}`, {
            method: "GET"
        });
...
async function buscarViewTecido(usuarioId){
    try {
        const resposta = await fetch(`http://10.0.1.226:8080/askyu/tecido-view/${usuarioId}`, {
            method: "GET"
        });
