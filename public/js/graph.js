async function buscarViewPedido(usuarioId){
    try {
        const resposta = await fetch(`http://localhost:8080/askyu/askyu/pedido-view/${usuarioId}`, {
            method: "GET"
        });
...
async function buscarViewTecido(usuarioId){
    try {
        const resposta = await fetch(`http://localhost:8080/askyu/askyu/tecido-view/${usuarioId}`, {
            method: "GET"
        });
