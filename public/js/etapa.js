...
async function buscarAtivo(usuarioId, etapaId) {
  try {
    const response = await fetch(`http://10.0.1.226:8080/askyu/agendamento/etapa-ativo/${usuarioId}/${etapaId}`, {
      method: "GET"
    });
...
async function buscarInativo(usuarioId, etapaId) {
  try {
    const response = await fetch(` http://10.0.1.226:8080/askyu/agendamento/etapa-inativo/${usuarioId}/${etapaId}`, {
      method: "GET"
    }
    );
...
async function buscarTotalEtapa() {
  const usuarioId = sessionStorage.getItem("id")
  const response = await fetch(`http://10.0.1.226:8080/askyu/agendamento-view-total-etapa/${usuarioId}`, {
    method: "GET"
  });
...
async function buscaAvancada(input){
  ...
  try{
    const response = await fetch(`http://10.0.1.226:8080/askyu/agendamento/filtro-cliente-nome/${usuarioId}?nome=${nomeCliente}&ativo=${valorSwitch}`, {
      method: "GET"
    });
