...
async function buscarAtivo(usuarioId, etapaId) {
  try {
    const response = await fetch(`/askyuagendamento/etapa-ativo/${usuarioId}/${etapaId}`, {
      method: "GET"
    });
...
async function buscarInativo(usuarioId, etapaId) {
  try {
    const response = await fetch(` /askyuagendamento/etapa-inativo/${usuarioId}/${etapaId}`, {
      method: "GET"
    }
    );
...
async function buscarTotalEtapa() {
  const usuarioId = sessionStorage.getItem("id")
  const response = await fetch(`/askyuagendamento-view-total-etapa/${usuarioId}`, {
    method: "GET"
  });
...
async function buscaAvancada(input){
  ...
  try{
    const response = await fetch(`/askyuagendamento/filtro-cliente-nome/${usuarioId}?nome=${nomeCliente}&ativo=${valorSwitch}`, {
      method: "GET"
    });
