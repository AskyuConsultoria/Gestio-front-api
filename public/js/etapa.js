...
async function buscarAtivo(usuarioId, etapaId) {
  try {
    const response = await fetch(`http://192.168.137.199:8080/agendamento/etapa-ativo/${usuarioId}/${etapaId}`, {
      method: "GET"
    });
...
async function buscarInativo(usuarioId, etapaId) {
  try {
    const response = await fetch(` http://192.168.137.199:8080/agendamento/etapa-inativo/${usuarioId}/${etapaId}`, {
      method: "GET"
    }
    );
...
async function buscarTotalEtapa() {
  const usuarioId = sessionStorage.getItem("id")
  const response = await fetch(`http://192.168.137.199:8080/agendamento-view-total-etapa/${usuarioId}`, {
    method: "GET"
  });
...
async function buscaAvancada(input){
  ...
  try{
    const response = await fetch(`http://192.168.137.199:8080/agendamento/filtro-cliente-nome/${usuarioId}?nome=${nomeCliente}&ativo=${valorSwitch}`, {
      method: "GET"
    });
