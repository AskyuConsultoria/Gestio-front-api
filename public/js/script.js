  ...
  async function login(){
    ...
    const data = await fetch(`http://10.0.1.226:8080/askyu/usuarios/login?usuario=${user}&senha=${senha}`);
      if (!data.ok) {
        if (data.status === 404) {
          ...
          await fetch(`http://10.0.1.226:8080/askyu/usuarios/login/deslogar?usuario=${user}`, {
            method: "POST" ,
