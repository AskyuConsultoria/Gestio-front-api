  ...
  async function login(){
    ...
    const data = await fetch(`http://localhost:8080/askyu/askyu/usuarios/login?usuario=${user}&senha=${senha}`);
      if (!data.ok) {
        if (data.status === 404) {
          ...
          await fetch(`http://localhost:8080/askyu/askyu/usuarios/login/deslogar?usuario=${user}`, {
            method: "POST" ,
