async function buscarPedidosPorAgendamentoId(){
    ...
    try{
        const reponse = await fetch(`http://localhost:8080/askyu/askyu/pedido/buscar-por-agendamento/${usuarioId}/${agendamentoId}`, {
            method: "GET",
        });
...
async function excluirPedidoPorId(evento, pedidoId){
    const usuarioId = sessionStorage.getItem("id")
    try{
        const response = await fetch(`http://localhost:8080/askyu/askyu/pedido/${usuarioId}/${pedidoId}`, {
            method: "DELETE"
        })
