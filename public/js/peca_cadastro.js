async function listarPecas() {
    ...
    const resposta = await fetch(`http://10.0.1.226:8080/askyu/askyu/pecas/${usuario}`);
...
async function cadastrarPeca() {
    ...
    const respostaCadastro = await fetch(`http://10.0.1.226:8080/askyu/askyu/pecas/${usuario}`, {
        method: "POST",
...
async function deletarPeca(id) {
    const usuario = sessionStorage.getItem("id");
    const resposta = await fetch(`http://10.0.1.226:8080/askyu/askyu/pecas/${usuario}/${id}`, {
        method: "DELETE"
    });
