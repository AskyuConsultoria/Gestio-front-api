async function listarPecas(){
    ...
    const data = await fetch(`/askyupecas/${id}`);
        if (!data.ok) {
...
async function listarMedidas() {
    ...
    const data1 = await fetch(`/askyupecas/${usuario}/${idPeca}`);
    if (!data1.ok) {
    ...
    const data = await fetch(`/askyunomes-medidas/${usuario}/${idPeca}`);
    if (!data.ok) {
        throw new Error('Erro ' + data.statusText);
    }
...
async function descEditar(){
    ...
    const data = await fetch(`/askyupecas/${usuario}/${id}`);
        if (!data.ok) {
        throw new Error('Erro ' + data.statusText);
    }
...
async function editarPeca(){
    ...
    const respostaCadastro = await fetch(`/askyupecas/${usuario}/${idPeca}`, {
    method: "PUT" ,
...
async function deletarMedida(idMedida){
    ...
    const respostaMedida = await fetch(`/askyunomes-medidas/${usuario}/${idPeca}/${idMedida}`, {
    method: "DELETE",
    headers: {"Content-type": "application/json; charset=UTF-8"}
})
