async function listarPecas(){
    ...
    const data = await fetch(`http://10.0.1.226:8080/askyu/pecas/${id}`);
        if (!data.ok) {
...
async function listarMedidas() {
    ...
    const data1 = await fetch(`http://10.0.1.226:8080/askyu/pecas/${usuario}/${idPeca}`);
    if (!data1.ok) {
    ...
    const data = await fetch(`http://10.0.1.226:8080/askyu/nomes-medidas/${usuario}/${idPeca}`);
    if (!data.ok) {
        throw new Error('Erro ' + data.statusText);
    }
...
async function descEditar(){
    ...
    const data = await fetch(`http://10.0.1.226:8080/askyu/pecas/${usuario}/${id}`);
        if (!data.ok) {
        throw new Error('Erro ' + data.statusText);
    }
...
async function editarPeca(){
    ...
    const respostaCadastro = await fetch(`http://10.0.1.226:8080/askyu/pecas/${usuario}/${idPeca}`, {
    method: "PUT" ,
...
async function deletarMedida(idMedida){
    ...
    const respostaMedida = await fetch(`http://10.0.1.226:8080/askyu/nomes-medidas/${usuario}/${idPeca}/${idMedida}`, {
    method: "DELETE",
    headers: {"Content-type": "application/json; charset=UTF-8"}
})
